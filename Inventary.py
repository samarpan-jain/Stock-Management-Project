...
def Open():
    ...
    y= t2.get()
    z= c1.get()
    if x=="admin" and y=="abc123@G" and z=="Purchase":
        root.withdraw()
        toplevel1(Mp)
    elif x=="admin" and y=="abc123@G" and z=="Sales":
        root.withdraw()
        toplevel5(Sa)
    else:
        pass
...
def toplevel4(top):
    ...
    for i in mycursor:
        ...
        l7.place(relx=0.132, rely=0.807, relheight=0.025, relwidth=0.24)
...
def toplevel5(top1):
    ...
    Button2.place(relx=0.215, rely=0.797, height=60, width=220)
    ...
    Button3.place(relx=0.415, rely=0.797, height=60, width=220)
    ...
    Button4.place(relx=0.612, rely=0.797, height=60, width=220)
    ...
    Button5.place(relx=0.809, rely=0.797, height=60, width=220)
...
def As(top):
    ...
    if(len(k)==0):
        ...
        l6=tk.Label(top,text="Date:",font=("arial",20,"bold"))
        ...
        for col in cols:
            ...
            listBox4.column(col,minwidth=0,width=100)
...
def cb(top):
    ...
    def onclosing():
        Msg=tk.messagebox.askyesno("Confirmation","Are you sure you want to close this window?",icon="warning",parent=top)
        if Msg==True:
            top.withdraw()
            mycursor.execute("Drop table if exists sale3")
        else:
...
def delete_btn4():
    try:
        ...
        if MsgBox == "yes":
            ...
            print(k[0],k[3])
            z=[]
            mycursor.execute("SELECT * FROM sale WHERE ino=? AND dob=?",(str(k[0]),str(k[3])))
            for i in mycursor:
                z.append(i)
            for j in range(len(z)):
                mycursor.execute("UPDATE stock SET quantity=quantity+? WHERE pname2=? AND measure=?",(z[j][3],z[j][2],z[j][4]))
