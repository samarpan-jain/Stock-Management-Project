...
try:
    ...
    mycursor.execute("create table supplier(name text, pname text,cnum int, date1 date, mop text, sadd text, sp text)")

try:
    mycursor.execute('select * from stock')
except(OperationalError):
    mycursor.execute("create table stock(srno int, pname2 text,quantity int, measure text, pi int)")

try:
    mycursor.execute('select * from purchase')
except(OperationalError):
    mycursor.execute("create table purchase(pname3 text,Quantity int,Measurement text, Price int, name2 text , date2 date, City text)")

try:
    mycursor.execute('select * from sale')
except(OperationalError):
    mycursor.execute("create table sale(ino int,dob date, pname5 text,Quantity2 int,Measurement2 text, rate1 int , discount1 int, Price2 int)")

try:
    mycursor.execute('select * from sale2')
except(OperationalError):
    mycursor.execute("create table sale2(ino1 int, cname text,cadd text,dob1 date, nop int , tq int, td int ,ta int)")

try:
    mycursor.execute('select * from sale3')
except(OperationalError):
    mycursor.execute("create table sale3(pname4 text,Quantity1 int,Measurement1 text, rate int , discount int, Price1 int)")
...
def toplevel2(top):
    ...
    for i in mycursor:
        ...
        l6.place(relx=0.046, rely=0.797, relheight=0.025, relwidth=0.10)
        l7.place(relx=0.146, rely=0.797, relheight=0.025, relwidth=0.02)
...
def toplevel4(top):
    ...
    for i in mycursor:
        ...
        l7=tk.Label(top,text=str(i[0]),font=("arial",15,"bold"))
        l6.place(relx=0.046, rely=0.807, relheight=0.025, relwidth=0.10)
        l7.place(relx=0.146, rely=0.807, relheight=0.025, relwidth=0.02)
...
def Add2(top):
    ...
    for i in mycursor:
        name2.set(i[0])
    mycursor.execute("select date2 from purchase order by date2 desc limit 1")
    for i in mycursor:
        date2.set(i[0])
...
def ADD(top):
    ...
    Entry4 = tk.Entry(top,width=28, textvariable=date1)
    Entry4.grid(row=5,column=1,pady=20,padx=20,sticky="w")
    Entry4.configure(font="-family {Palatino Linotype} -size 12")
    ...
    try:
        if (name2.get() == "" or pname3.get() == "" or Quantity.get() == "" or Measurement.get()=="" or Price.get()=="" or date2.get()=="" or City.get() == ""):
            ...
            if (name2.get() == ""):
                ...
                t=[]
                mycursor.execute("select sadd,count(*) from supplier where name=? GROUP BY name",(name2.get(),))
                for i in mycursor:
                    t.append(i)
                if len(t)==0:
            ...
            if (City.get() == ""):
                ...
                t=[]
                mycursor.execute("select name,count(*) from supplier where sadd=? GROUP BY sadd",(City.get(),))
                for i in mycursor:
                    t.append(i)
                if len(t)==0:
            ...
            try:
                ...
                if (date1.get() == ""):
                    ...
                    try:
                        ...
                        date1.set("Please Fill the date carefully!")
                if (mop.get() == ""):
                    list1.append('Null')
                else:
...
def search_btn(top):
    if top==Ms:
        ...
        if (_option == 'Supplier Name'):
            ...
            listBox.delete(*listBox.get_children())
            mycursor.execute("select * from supplier where date1='{0}'".format(_entry))
            for i in mycursor:
                listBox.insert("", "end", values=i)
...
def cb(top):
    ...
    l8=tk.Label(top,text="Date(dd/mm/yy):",font=("arial",20,"bold"))
    l8.place(relx=0.668, rely=0.114, relheight=0.069, relwidth=0.149)
    r6=tk.StringVar()
    Entry2=tk.Entry(top,width=20,textvariable=r6)
    Entry2.place(relx=0.810, rely=0.136, relheight=0.032, relwidth=0.142)
    Entry2.configure(background="white")
    Entry2.configure(disabledforeground="#a3a3a3")
    Entry2.configure(font="-family {Palatino Linotype} -size 15")
    Entry2.configure(foreground="#000000")
    ...
    listBox5.place(relx=0.005, rely=0.269, relheight=0.580, relwidth=0.750)
    for col in cols:
        listBox5.heading(col, text=col)
        
    vsb = tt.Scrollbar(top, orient="vertical", command=listBox5.yview)
    vsb.place(relx=0.755, rely=0.270, relheight=0.580, relwidth=0.012)
    listBox5.configure(yscrollcommand = vsb.set)

    
    sbframe1=tk.Frame(top)
    sbframe1.place(relx=0.021, rely=0.867, height=60, width=750)
...
def saveadd3(tp):
    ...
    try:
        if (pname4.get() == "" or Quantity1.get() == "" or Measurement1.get() == "" or rate.get() == ""):
            ...
            mycursor.execute("UPDATE stock SET Quantity=Quantity-? where pname2=? AND measure=?",(int(list1[1]),list1[0],list1[2]))
            conn.commit()
            clear3()
...
def delete_btn3():
    try:
        ...
        if MsgBox == "yes":
            ...
            mycursor.execute("UPDATE stock SET quantity=quantity+? WHERE pname2=? AND measure=?",(str(k[1]),str(k[0]),str(k[2])))
            conn.commit()
            for selected_item in selected_items:
...
def saveandcalc():
    try:
        ...
        if(len(k)==0):
            ...
            if r7.get()=="" or r6.get()=="" or r12.get()=="" or r11.get()=="":
                ...
                list2=[]
                j1=[]
                mycursor.execute("select * from sale2 where ino1=? AND dob1=?",(r7.get(),r6.get(),))
                for i in mycursor:
                    j1.append(i[0])
                if (len(j1)!=0):
                    ...
                    if (r6.get() == ""):
                        r6.set("Please fill all the fields correctly!")
                         ...
                         try:
                            ...
                            r6.set("Please Fill the date carefully!")
                    a=[]
